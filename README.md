...
2. **Name your new repository** using the convention: `Northern Mariana Islands Data Pipeline` (e.g., `il-data-pipeline`, `tx-data-pipeline`).
...
3. **Update the state abbreviation** in both workflow files:
   ...
   **In `.github/workflows/scrape-and-format-data.yml`:**
   ...
   ```yaml
   env:
     STATE_CODE: mp # CHANGE THIS to your state abbreviation
   ...
   **In `.github/workflows/extract-text.yml`:**
   ...
   - name: Extract text
     ...
     with:
       state: mp # CHANGE THIS to your state abbreviation
   ```
...
```
Northern Mariana Islands Data Pipeline/
├── .github/workflows/
...
uses: windy-civi/toolkit/actions/scrape@main
with:
  state: mp # State abbreviation (required)
...
git clone https://github.com/YOUR-ORG/Northern Mariana Islands Data Pipeline
cd Northern Mariana Islands Data Pipeline
